...
parser.add_argument('--sample_interval', type=int,
                    default=1000, help='interval between image sampling')
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear((opt.n_classes + int(np.prod(img_shape))), 512),
                                   ...
                                   nn.Dropout(0.4),
                                   nn.LeakyReLU(0.2),
                                   nn.Linear(512, 2)  # 添加最后一个线性层，最终输出为一个实数
                                   )
...
def sample_image(n_row, batches_done):
    ...
    labels = jt.array(np.array([num for _ in range(n_row)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # 采样随机噪声和数字类别作为生成器输入
        z = jt.array(np.random.normal(
            0, 1, (batch_size, opt.latent_dim))).float32()
        gen_labels = jt.array(np.random.randint(
            0, opt.n_classes, batch_size)).float32()
...
labels = jt.array(np.array([int(number[num])
                  for num in range(n_row)])).float32().stop_grad()
gen_imgs = generator(z, labels)

img_array = gen_imgs.data.transpose(
    (1, 2, 0, 3))[0].reshape((gen_imgs.shape[2], -1))
min_ = img_array.min()
max_ = img_array.max()
